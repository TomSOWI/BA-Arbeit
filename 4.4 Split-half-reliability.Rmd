...
check_df_party1 <- function(df){
    ...
    if (!any(df$party == "CDU/CSU")) {
      df[nrow(df) + 1,] = list("CDU/CSU",0,0,0,0,0)
    }
    if (!any(df$party == "AfD")){
      df[nrow(df) + 1,] = list("AfD",0,0,0,0,0)
    }
    if (!any(df$party == "FDP")){
      df[nrow(df) + 1,] = list("FDP",0,0,0,0,0)
    }
    if (!any(df$party == "SPD")){
      df[nrow(df) + 1,] = c("SPD",0,0,0,0,0)
    }
    if (!any(df$party == "LINKE")){
      df[nrow(df) + 1,] = list("LINKE",0,0,0,0,0)
    }
    if (!any(df$party == "Grüne")){
      df[nrow(df) + 1,] = list("Grüne",0,0,0,0,0)
    }
    df[order(df$party), ]
    return(df)
  }
...
```{r}
speechesRelia <- data.frame(gruendl = unlist(reliaGruendl), clara = unlist(reliaClara), claraK = unlist(reliaClaraK), kmeans = unlist(reliaKmeans), kmeansK = unlist(reliaKmeansK))
...
```{r}
speeches10Relia <- data.frame(gruendl = unlist(reliaGruendl), clara = unlist(reliaClara), gruendl_10 = unlist(reliaGruendl_speeches10), clara_10 = unlist(reliaClara_speeches10))
