...
corp_2014 <- corpus(testing, text_field = "speech_content") #df_2014
...
#return_value = "count"
result1 <- popdictR::run_popdict(corp_2014_clean, dict_version = "current", at_level = "documents", return_value = "count")
#count hits and not per sentence
result1 <- popdictR::run_popdict(corp_2014, dict_version = "current", at_level = "documents", return_value = "count") 
...
t <- result1_2014 == result2_2014

table(t)

sum(result1_2014$dict_gruendl_2020)

t <- result1_2014[result1_2014$doc_id == "text3",]
t$text
...
toks2014 <- tokens_tolower(toks2014)
toks2014 <- tokens_remove(toks2014, stopwords("german"))
corp_2014_clean <- vapply(toks2014, paste, FUN.VALUE = character(1), collapse = " ") %>%
  corpus()

#dictionary implementation
#dict_regex$populismus
#t.1
df_kwic_regex1 <- kwic(corp_2014_clean, pattern = "finan(c|z)ier(s|e)?" , valuetype = "regex", window = 1,  case_insensitive = T)
df_kwic_regex2 <- kwic(corp_2014_clean, pattern = "finan(c|z)ier(s|e)?" , valuetype = "regex", window = 1,  case_insensitive = F)
df_kwic_regex2 <- kwic(corp_2014, pattern = dict_regex$populismus, valuetype = "regex", window = 1)#, case_insensitive = FALSE) #4752
df_kwic_fixed <- kwic(corp_2014, pattern = phrase(dict_fixed$populismus), valuetype = "fixed", window = 1) #case_insensitive = FALSE) 
#211
df_kwic <- rbind(df_kwic_regex2,df_kwic_fixed)

table(df_kwic_regex1$pattern)
table(df_kwic_regex2$pattern)
...
result_2014_weight <- merge(result_2014, df_res, by = "doc_id") #df_res1
...
### Test ohne Gewichtung
```{r}
df_kwic1<- kwic(corp_2014, pattern = dict, valuetype = "regex", window = 1) #case_insensitive = FALSE)
df_kwic2<- kwic(corp_2014, pattern = dict$populismus, valuetype = "regex", window = 1, case_insensitive = F)
df_kwic3<- kwic(corp_2014, pattern = dict$populismus, valuetype = "regex", window = 1, case_insensitive = T)
df_kwic1.1<- kwic(corp_2014, pattern = dict, valuetype = "regex", window = 1, case_insensetive = FALSE)
df_kwic1.2<- kwic(corp_2014, pattern = dict, valuetype = "regex", window = 1, case_insensetive = TRUE)
...
df_kwic_test <- df_kwic

df_kwic_test$weight <- 1

df_kwic_test <- as.data.frame(df_kwic_test)

#hier ist der Filter
df_res1 <- df_kwic_test %>%
  select(docname,weight)%>%
  group_by(df_res$docname)%>%
  reframe(
    weight = sum(weight),
...
df_res1$`df_res$docname`
```

```{r}
t <- result_2014[result_2014$doc_id == "text4",]




t.dfm <- dfm(tokens(t$text))

t <- dfm_lookup(t.dfm, dictionary = dict_fixed)
