...
# "-" kann entfernt werden, da es keinen Effekt auf Pattern hat
...
```{r}
mp2013Clean <- expertyears_cleaning(corp = mp2013, pattern = popdictR::gruendl_terms)

#mp2013Clean <- tokens_remove(mp2013Clean, c("__","c"))
...
terms_fixed <- terms_fixed$Word
terms_fixed <- stringi::stri_replace_all_fixed(terms_fixed," ", "_")
terms_regex <- terms_regex$Word
```

```{r}

terms_fixed  <- paste("\\b(_)?",terms_fixed ,"(_)?\\b",sep = "")

df_kwic <- kwic(mp2013Clean[[6]], pattern = terms_fixed, valuetype = "regex", window = 1, case_insensitive = T)
df_kwic2 <- kwic(mp2013Clean, pattern = terms_fixed, valuetype = "regex", window = 1, case_insensitive = T)


#df_kwic2 <- kwic(mp2013Clean, pattern = pattern_us1, valuetype = "regex", window = 1, case_insensitive = T)
```

\b(_)?lug(_)?\b

```{r}
pattern_us <- gruendl_patterns_to_underscore()


df_kwic_normal <- kwic(mp2013Clean[[6]], pattern = gruendl_patterns_to_underscore(), valuetype = "regex", window = 1, case_insensitive = T)
df_kwic_normal2 <- kwic(mp2013Clean, pattern = gruendl_patterns_to_underscore(), valuetype = "regex", window = 1, case_insensitive = T)
...
pattern_us1 <- paste("(_)?",pattern_us,"(_)?",sep = "")

df_kwic <- kwic(mp2013Clean[[6]], pattern = pattern_us1, valuetype = "regex", window = 1, case_insensitive = T)
df_kwic2 <- kwic(mp2013Clean, pattern = pattern_us1, valuetype = "regex", window = 1, case_insensitive = T)
...
validation2013 <- merge(result2013, exp_2014_afd)
...
```{r}
mp2017Clean <- expertyears_cleaning(corp = mp2017, pattern = popdictR::gruendl_terms)

#mp2017Clean <- tokens_remove(mp2017Clean, c("__","c"))
...
```{r}
mp2021Clean <- expertyears_cleaning(corp = mp2021, pattern = popdictR::gruendl_terms)
...
result2013$'2013' <- round(result2013$'2013'/result2013$n_sentences * 100, digits = 3)
...
result2021$'2021' <- round(result2021$'2021'/result2021$n_sentences * 100, digits = 3)
